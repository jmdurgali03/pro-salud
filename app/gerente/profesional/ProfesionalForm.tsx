...
export default function ProfesionalForm({
  ...
  const [telefono, setTelefono] = useState((initialData?.telefono ?? "").replace(/\D/g, "").slice(0, 10));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!initialData) { data.dni = dni; data.matricula = matricula; }
    onSubmit(data);
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      {!initialData && (
        <>
          <input type="text" value={dni} onChange={(e) => setDni(e.target.value.replace(/\D/g, "").slice(0, 8))}
            placeholder="DNI (8 dígitos)" className="w-full border rounded px-3 py-2" required maxLength={8}
            pattern="\d{8}" title="Debe contener 8 dígitos" />
          <input type="text" value={matricula} onChange={(e) => setMatricula(e.target.value.replace(/\D/g, "").slice(0, 4))}
            placeholder="Matrícula (4 dígitos)" className="w-full border rounded px-3 py-2" required maxLength={4}
            pattern="\d{4}" title="Debe contener 4 dígitos" />
        </>
      )}

      <select value={especialidadId} onChange={(e) => setEspecialidadId(e.target.value as Id<"especialidades">)}
        className="w-full border rounded px-3 py-2" required>
        <option value="">Seleccionar especialidad</option>
        {especialidades.map((esp) => <option key={esp._id} value={esp._id}>{esp.nombre}</option>)}
      </select>

      <input type="email" value={contacto} onChange={(e) => setContacto(e.target.value)}
        placeholder="Contacto (ej: usuario@gmail.com)" className="w-full border rounded px-3 py-2" required />

      <input type="tel" inputMode="numeric" value={telefono}
        onChange={(e) => setTelefono(e.target.value.replace(/\D/g, "").slice(0, 10))}
        placeholder="Teléfono (10 dígitos)" className="w-full border rounded px-3 py-2" required
        maxLength={10} pattern="\d{10}" title="Debe contener exactamente 10 dígitos" />

      <div className="space-y-2">
        <label className="font-medium text-gray-700">Obras Sociales</label>
        {obrasSociales.map((os) => (
          <div key={os._id} className="flex items-center gap-2">
            <input type="checkbox" checked={obrasSeleccionadas.includes(os._id)}
              onChange={() => handleObraSocialChange(os._id)} className="h-4 w-4 text-blue-600 border-gray-300 rounded" />
            <span>{os.nombre}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-end gap-2 pt-2">
        <button type="button" onClick={onCancel}
          className="px-3 py-2 rounded bg-gray-200 hover:bg-gray-300 disabled:opacity-60" disabled={submitting}>
          Cancelar
        </button>
        <button type="submit"
